...
public class ArmyInPlaceMeleeAttackGoal extends Goal
{
    ...
    public void tick() {
        ...
        if (this.armyEntity.getSensing().canSee(livingEntity) && this.updateCountdownTicks <= 0 && this.armyEntity.getRandom().nextFloat() < 0.05F) {
    ...
    protected void attack(LivingEntity target, double squaredDistance) {
        ...
        if (squaredDistance <= d && this.isCooledDown()) {
            if (!this.armyEntity.getCommandSenderWorld().isClientSide()) {
                double attackSpeed = this.armyEntity.getAttribute(SharedMonsterAttributes.ATTACK_SPEED).getValue();
