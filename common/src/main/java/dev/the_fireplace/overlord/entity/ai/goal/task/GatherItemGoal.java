...
public class GatherItemGoal extends TaskGoal
{
    ...
    protected boolean hasItemTarget() {
        return item != null
            && item.isAlive()
            && !item.isInvisible()
            && this.armyEntity.isWithinRestriction(new BlockPos(item.position()));
    }
    ...
    private void gatherItem(double squaredDistanceToItem) {
        if (squaredDistanceToItem <= getSquaredMaxPickupDistance(item) && this.armyEntity.getOffhandItem().isEmpty()) {
            ...
            this.armyEntity.level.playSound(null, this.armyEntity, SoundEvents.ITEM_PICKUP, SoundSource.NEUTRAL, 0.2F, ((random.nextFloat() - random.nextFloat()) * 0.7F + 1.0F) * 2.0F);
    ...
    private void moveToItem(double squaredDistanceToCow) {
        if (this.armyEntity.getSensing().canSee(item)
            && this.updateCountdownTicks <= 0
