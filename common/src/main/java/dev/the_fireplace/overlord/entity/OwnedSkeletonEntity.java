...
public class OwnedSkeletonEntity extends ArmyEntity implements RangedAttackMob, CrossbowAttackMob, AnimatedMilkDrinker, AugmentBearer
{
    ...
    private void updateGrowthPhaseAdvancement() {
        ...
        if (hasGrownSinceLastAdvancementGranted && this.getOwner() instanceof ServerPlayer) {
    ...
    public boolean mobInteract(Player player, InteractionHand hand) {
        if (!player.isShiftKeyDown()) {
            if (!player.level.isClientSide() && player.getUUID().equals(getOwnerUUID())) {
                ...
                menuLoaderHelper.openMenu(OverlordEntities.OWNED_SKELETON_ID, player, buf -> buf.writeUUID(this.getUUID()));
            }
            return true;
        }
    ...
    public void die(DamageSource source) {
        ...
        if (source != null) {
            this.setDeltaMovement(
                -Mth.cos((this.hurtDir + this.yRot) * (float) Math.PI / 180) * 0.1f,
                0.1f,
                -Mth.sin((this.hurtDir + this.yRot) * (float) Math.PI / 180) * 0.1f
            );
        } else {
    ...
    public void setHasMuscles(boolean hasMuscles) {
        ...
        this.getAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).removeModifier(MUSCLE_ATTACK_BONUS);
        this.getAttribute(SharedMonsterAttributes.ARMOR_TOUGHNESS).removeModifier(MUSCLE_TOUGHNESS_BONUS);
        this.getAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).removeModifier(MUSCLE_SPEED_BONUS);
        if (hasMuscles) {
            this.getAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).addModifier(MUSCLE_ATTACK_BONUS);
            this.getAttribute(SharedMonsterAttributes.ARMOR_TOUGHNESS).addModifier(MUSCLE_TOUGHNESS_BONUS);
            this.getAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).addModifier(MUSCLE_SPEED_BONUS);
        }
    }
    ...
    @Override
    public void shootProjectile(LivingEntity target, ItemStack crossbow, Projectile projectile, float multiShotSpray) {
    ...
    private void shootCrossbow(LivingEntity target, ItemStack crossbow, Projectile projectile, float multiShotSpray) {
        ...
        double g = target.getY(1.0 / 3.0) - ((Entity) projectile).getY() + f * 0.2;
