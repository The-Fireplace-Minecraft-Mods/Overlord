...
public class GatherMilkGoal extends TaskGoal
{
        ...
        return armyEntity.getCommandSenderWorld().getNearestEntity(
            ...
            new TargetingConditions().selector(
                cow -> !cow.isBaby()
                    ...
                    && this.armyEntity.isWithinRestriction(new BlockPos(cow.position()))
            ),
            armyEntity,
            armyEntity.getX(),
            armyEntity.getY(),
    ...
    private void moveToCow(double squaredDistanceToCow) {
        if (this.armyEntity.getSensing().canSee(cow)
            && this.updateCountdownTicks <= 0
    ...
    private boolean swapToBucket() {
        if (!armyEntity.getOffhandItem().getItem().equals(Items.BUCKET)) {
