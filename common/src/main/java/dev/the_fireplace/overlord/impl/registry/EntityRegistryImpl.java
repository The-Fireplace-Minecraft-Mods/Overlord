...
public final class EntityRegistryImpl implements EntityRegistry
{
    ...
    public EntityRegistryImpl() {
        if (BuiltInRegistries.ENTITY_TYPE.stream().toArray().length == 0) {
        ...
        for (EntityType<?> entityType : BuiltInRegistries.ENTITY_TYPE) {
            if (!entityType.getCategory().isFriendly()) {
                mobIds.add(BuiltInRegistries.ENTITY_TYPE.getKey(entityType));
            } else {
                animalIds.add(BuiltInRegistries.ENTITY_TYPE.getKey(entityType));
            }
        }
    }
