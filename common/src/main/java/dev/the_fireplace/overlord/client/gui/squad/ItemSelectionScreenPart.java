...
public class ItemSelectionScreenPart implements PartialScreen
{
    ...
    @Override
    public <T extends GuiEventListener & Widget> List<T> getChildren() {
    ...
    private void createPageChangeButtons() {
        previousButton = new Button(x, y, this.width / 2 - 2, 20, I18n.get("gui.overlord.create_squad.previous"), buttonWidget -> {
            this.state.currentPage--;
            updatePageChangeButtonUsability();
            updateItemVisibility();
        });
        nextButton = new Button(x + width / 2 + 4, y, this.width / 2 - 6, 20, I18n.get("gui.overlord.create_squad.next"), buttonWidget -> {
            this.state.currentPage++;
            updatePageChangeButtonUsability();
            updateItemVisibility();
        });
