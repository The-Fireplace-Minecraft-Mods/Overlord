...
public class AugmentHeadFeatureRenderer<T extends OwnedSkeletonEntity, M extends EntityModel<T> & HeadedModel> extends RenderLayer<T, M>
{
    ...
    public AugmentHeadFeatureRenderer(RenderLayerParent<T, M> context) {
        super(context);
    ...
    public void render(PoseStack matrixStack, MultiBufferSource vertexConsumerProvider, int light, T livingEntity, float f, float g, float h, float j, float k, float l) {
        ...
        if (!itemStack.isEmpty()) {
            ...
            if (item instanceof BlockItem && ((BlockItem) item).getBlock() instanceof AbstractSkullBlock) {
                ...
                SkullModel skullBlockEntityModel = this.headModels.get(skullType);
                RenderType renderLayer = SkullBlockEntityRendererAccessor.callGetRenderType(skullType, gameProfile);
                ...
                SkullModel skullBlockEntityModel = this.armyHeadModels.get(skullType);
     ...
     */
    public static void renderVanillaSkull(int overlay, float yaw, float animationProgress, PoseStack matrices, MultiBufferSource vertexConsumers, int light, SkullModel model, RenderType renderLayer) {
        matrices.pushPose();
