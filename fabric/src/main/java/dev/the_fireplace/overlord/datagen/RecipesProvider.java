...
public class RecipesProvider extends FabricRecipeProvider
{
    ...
    protected void generateRecipes(Consumer<FinishedRecipe> consumer) {
        ...
        ShapedRecipeBuilder.shaped(overlordBlocks.getMangroveCasket(), 1).define('#', Items.STRIPPED_MANGROVE_WOOD).define('%', Items.MANGROVE_PLANKS).define('B', bedIngredient).pattern("###").pattern("%B%").pattern("###").group("wooden_casket").unlockedBy("has_stripped_wood", this.conditionsFromItem(Blocks.STRIPPED_MANGROVE_WOOD)).save(consumer);
        ...
        ShapedRecipeBuilder.shaped(overlordBlocks.getMangroveGraveMarker(), 2).define('#', Items.MANGROVE_FENCE).define('%', Items.MANGROVE_SLAB).pattern("%%%").pattern(" # ").pattern(" # ").group("grave_marker").unlockedBy("has_fence", this.conditionsFromItem(Items.MANGROVE_FENCE)).save(consumer);
