...
{
    private final Map<Identifier, Pattern> patterns = new LinkedHashMap<>();
    ...
    private Pattern createSimple(String patternPath) {
        return new PatternImpl(new Identifier(Overlord.MODID, patternPath));
    }
    ...
    @Override
    public Pattern getById(Identifier id) {
        return patterns.getOrDefault(id, new MissingPattern());
    }

    @Override
    public boolean hasPattern(Identifier id) {
